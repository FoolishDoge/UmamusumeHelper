...
<li>아이템 고정기능: 체크할 경우 아이템 아이콘에 핀셋 아이콘이 활성화됨. 눌러서 색이 들어올 경우 '전체' 태그인 상태에서 검색화면이 비어있을 때 표시됨.</li>
...
const TIP6_DESC = `
서포트 카드의 가장 중요한 스탯들은 다음과 같다.<br>
<ul>
<li>트레이닝 효과 증가: 함께 훈련할 때 스탯 상승치가 더 높아지는 스탯. 담당 훈련에 있지 않아도 적용되기 때문에 고점은 물론 육성의 안정성까지 매우 크게 높여준다.</li>
<li>좋은 스킬을 가지고 있는 지의 여부: 좋은 스킬은 레이스의 승패를 결정짓기 때문에 매우 중요하다.</li>
<li>특기율: 담당 훈련에 배치될 확률이 보정되는 스탯. 육성은 우정 트레이닝을 얼마나 띄우느냐가 핵심이므로 매우 중요하다. 참고로 담당 훈련에 배치될 확률은 '0.18 × (1 + (특기율 / 100))' 정도로 추정되고 있다. 특기율이 100인 SSR 키타산 블랙 풀돌의 경우 대략 3번 중 1번은 출석한다는 뜻.</li></ul>
`;
